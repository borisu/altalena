...
void 
ICall::OfferType(const string &offer_type)
{
...
{
	LogDebug("ICall::ResetState iwh:" << _iwCallHandle << ", transition from state:" << _callState << " to state:" << state << ", " << state_str);
	_callState = state;
}

ApiErrorCode
ICall::MakeCall(IN const std::string &destination_uri, 
				 IN const std::string &offer,
				 IN csp::Time ring_timeout)
{
	FUNCTRACKER;

	LogDebug("ICall::MakeCall dst:" << destination_uri <<", offer:\n" << offer);
...
void
IMediaCall::EnableMediaFormat(IN const MediaFormat& media_format)
{
	FUNCTRACKER;

	LogDebug("IMediaCall::EnableMediaFormat - Media format:" << media_format << ", enabled for iwh:" << _iwCallHandle );
...
ApiErrorCode
IMediaCall::NegotiateMediaFormats(IN const MediaFormatsList &offered_medias, 
							   ...
							   OUT MediaFormat &accepted_speech_format)
{
	...
	for (MediaFormatsList::const_iterator i = offered_medias.begin();
		...
		++i)
	{
		LogDebug("IMediaCall::EnableMediaFormat - offered:" << (*i).sdp_name_tos() << ", iwh:" << _iwCallHandle);
	}
	...
	for (MediaFormatsMap::const_iterator i = _supportedMediaFormatsMap.begin();
		...
		++i)
	{
		LogDebug("IMediaCall::EnableMediaFormat enabled:" << (*i).second.sdp_name_tos() << ", iwh:" << _iwCallHandle);
	}

	if (_supportedMediaFormatsMap.empty() || offered_medias.empty())
	{
		LogWarn("IMediaCall::EnableMediaFormat - Illegal parameters (empty?) for codecs negotiation.");
		return API_FAILURE;
	}
	...
	{
		LogWarn("IMediaCall::EnableMediaFormat - negotiation failure iwh:" << _iwCallHandle );
		return API_FAILURE;
	}

	LogDebug("IMediaCall::EnableMediaFormat speech:" << accepted_speech_format << ", rc2833 dtmf chosen:" << dtmf_chosen << ", iwh:" << _iwCallHandle);
...
IMediaCall::IMediaCall(IN ScopedForking &forking, 
		   IN HandleId stack_handle_id)
:ICall(forking, stack_handle_id)
{
	FUNCTRACKER;
}
...
IMediaCall::IMediaCall(IN ScopedForking &forking,
		   IN HandleId stack_handle_id,
		   IN shared_ptr<MsgCallOfferedReq> offered_msg):
ICall(forking,stack_handle_id, offered_msg),
...
ApiErrorCode
IMediaCall::AcceptInitialOffer( IN const CnxInfo &local_connection, 
							...
							IN const MediaFormat &speech_codec)
{
	...
	LogDebug("IMediaCall::AcceptInitialOffer iwh:" << _iwCallHandle);
...
ApiErrorCode
IMediaCall::MakeCall(IN const string &destination_uri, 
...
CnxInfo 
IMediaCall::RemoteMedia() const 
{ 
	return _remoteMedia; 
}

void 
IMediaCall::RemoteMedia(CnxInfo &val) 
{ 
	_remoteMedia = val; 
}
...
CnxInfo 
IMediaCall::LocalMedia() const 
{ 
	return _localMedia; 
}

void 
IMediaCall::LocalMedia(CnxInfo &val) 
{ 
	_localMedia = val; 
}
...
const MediaFormat& 
IMediaCall::AcceptedSpeechCodec()
{
