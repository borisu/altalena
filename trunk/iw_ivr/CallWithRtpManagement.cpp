...
namespace ivrworx
{
	...
	CallWithRtpManagement::AcceptInitialOffer()
	{
		...
		res = SipCall::AcceptInitialOffer(
	...
	CallWithRtpManagement::UponCallTerminated(IwMessagePtr ptr)
	{
		...
		SipCall::UponCallTerminated(ptr);
	}
	...
	CallWithRtpManagement::MakeCall(IN const string &destination_uri)
	{
		...
		res = SipCall::MakeCall(destination_uri,_callerRtpSession.LocalCnxInfo(),_ringTimeout);
		if (IW_FAILURE(res))
		{
			return res;
		};
