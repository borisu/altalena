...
{
#pragma region CCU2
	if (!ClientThread() && !JThread::IsRunning())
	{
	...
	while ((!ClientThread() && JThread::IsRunning()) && 
		(MIPTime::getCurrentTime().getValue() - curTime.getValue()) < 5.0) // wait maximum five seconds
	{
...
void MIPComponentChain::process(bool &done, 
								bool &error,
								std::string &errorComponent, 
								std::string &errorString,
								int64_t &iteration ,
								MIPSystemMessage &startMsg)
{
		...
		for (it = m_orderedConnections.begin() ; !error && it != m_orderedConnections.end() ; it++)
		{
			...
			do
			{
				...
				else
				{
					...
					{
						// CCU changes
						// if there is at least one message pulled we are not done
						// <<<<
						if (ExitOnEmptyChain())
						{
							done = false;
...
void *MIPComponentChain::Thread()
{
	...
	while (!done && !error)
	{
		process(done, error,errorComponent,errorString, iteration, startMsg);
	}
