	...
	public:
		...
		MsgVoipCallMixin():
		  ...
		  virtual void copy_data_on_response(IN IwMessage *msg)
		  {
			  ...
			  offer_type		= offer_type.empty() ? req->offer_type : offer_type;
			  provider_id		= provider_id.empty() ? req->provider_id : provider_id;
...
#define CALL_RESET_STATE(X) ResetState(X,#X)
	...
	IwStackHandle GenerateCallHandle();
	...
	public:
		...
		CallSession(IN ScopedForking &forking, 
		...
		virtual const string& OfferType();

		virtual void OfferType(const string &offer_type);
		...
		virtual CallState CurrentCallState() const;
	...
	public:
		...
		MediaCallSession(IN ScopedForking &forking, 
			  IN HandleId stack_handle_id);
	...
	protected:
		...
		BOOL _useGenericOfferAnswer;

	};

	typedef shared_ptr<MediaCallSession>
	MediaCallSessionPtr;
